...
int	check_args(int ac, char **av, t_main *main)
{
	...
	if (!main->input.nb_philo || !main->input.ttd || !main->input.tte
		...
		return (1);
	if (main->input.tts != main->input.tte && main->input.nb_philo % 2 == 0)
	{
		if (main->input.tte > main->input.tts)
			main->input.ttt = main->input.tte - main->input.tts;
		else
			main->input.ttt = main->input.tts - main->input.tte;
	}
	if (main->input.tts != main->input.tte && main->input.nb_philo % 2 == 1)
	{
		if (main->input.tte > main->input.tts)
			main->input.ttt = 2 * main->input.tte - main->input.tts;
		else
			main->input.ttt = 2 * main->input.tts - main->input.tte;
	}
	return (0);
}
